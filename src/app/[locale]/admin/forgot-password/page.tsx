    ...
    <div className="min-h-screen bg-[#FDFCFD] dark:bg-black flex flex-col items-center justify-center p-6 selection:bg-purple-100 selection:text-purple-900">
      <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <div className="text-center space-y-4">
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">¿Olvidaste tu contraseña?</h1>
          <p className="text-slate-500 dark:text-zinc-400 font-medium">No te preocupes, te enviaremos instrucciones para recuperarla.</p>
        </div>
        ...
        {message ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-slate-900 dark:bg-white dark:text-black text-white py-5 rounded-2xl font-black text-sm uppercase tracking-widest hover:opacity-90 transition-all shadow-2xl shadow-slate-900/20 dark:shadow-white/10 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3"
            >
        ...
        <div className="text-center text-xs text-slate-400 font-medium">
          &copy; {new Date().getFullYear()} ZyncSlot (ZincSlot) · Seguridad de Grado Industrial
        </div>
      </div>
    </div>
  );
