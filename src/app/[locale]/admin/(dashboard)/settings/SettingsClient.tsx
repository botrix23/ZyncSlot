...
const PLAN_LABELS: Record<string, string> = { BASIC: '1 admin', PROFESSIONAL: '2 admins', ENTERPRISE: 'ilimitados' };
...
export default function SettingsClient({
  ...
  const handleToggle = async (admin: Admin) => {
    ...
    if (res.success) {
      ...
      setError('No puedes desactivar el único administrador activo.');
    }
    setActionId(null);
  };
  ...
  const handleDelete = async (admin: Admin) => {
    ...
    if (res.success) {
      ...
      setError('No puedes eliminar el único administrador.');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setError(`Tu plan ${plan} permite máximo ${res.limit} administrador(es) activo(s). Actualiza tu plan para agregar más.`);
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Gestiona los administradores de tu cuenta.</p>
      </div>
      ...
      <div className="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/5 rounded-3xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-zinc-900 dark:text-white text-sm">Administradores</h2>
              <p className="text-xs text-zinc-500">Plan {plan} · {PLAN_LABELS[plan] ?? '1 admin'}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <span className={`text-xs font-bold px-2 py-1 rounded-full ${activeCount >= limit ? 'bg-rose-500/10 text-rose-500' : 'bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-400'}`}>
              {activeCount}{limit !== Infinity ? `/${limit}` : ''} activo{activeCount !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
          ...
          {tempPassword && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 space-y-2">
              <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold text-sm">
                ...
                Administrador creado — contraseña temporal (expira en 7 días)
              </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {admin.id === currentUserId && (
                      <span className="text-xs bg-purple-500/10 text-purple-600 dark:text-purple-400 font-bold px-1.5 py-0.5 rounded-md shrink-0">Tú</span>
                    )}
                  </div>
          ...
          {showCreate ? (
            <form onSubmit={handleCreate} className="space-y-3 border border-purple-500/20 bg-purple-500/5 rounded-2xl p-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  {creating ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Crear administrador'}
                </button>
              </div>
            </form>
          ) : (
            <button
              ...
              disabled={!canAdd}
              title={!canAdd ? `Tu plan ${plan} permite máximo ${limit} administrador(es) activo(s)` : ''}
