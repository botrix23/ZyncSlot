  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Directorio de clientes</h1>
          <p className="text-slate-500 dark:text-zinc-400 mt-1">Gestiona y fideliza a tus clientes frecuentes.</p>
        </div>
      ...
      {clientStats.length === 0 ? (
          <div className="text-center py-24 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/5 rounded-[32px] shadow-sm">
            ...
            <h3 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">Aún no tienes clientes registrados</h3>
            <p className="text-slate-500 dark:text-zinc-400">Las citas agendadas aparecerán automáticamente aquí.</p>
          </div>
      ) : (
                ...
                <table className="w-full text-left border-collapse">
                   <thead>
                      <tr className="border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase">Cliente</th>
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase">Contacto</th>
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase">Servicio principal</th>
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase text-center">Frecuencia</th>
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase text-right">Total gastado</th>
                         <th className="p-6 text-[10px] font-black tracking-widest text-slate-400 uppercase text-right">Acción</th>
                      </tr>
                   </thead>
                          ...
                          return (
                            <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => setSelectedClient(client)}>
                               <td className="p-6">
                                  <div className="flex items-center gap-3">
                                     ...
                                     <div className="flex flex-col">
                                        ...
                                        {isVip && <span className="text-[8px] font-black text-amber-600 uppercase tracking-tight">Frecuente</span>}
                                     </div>
                                  </div>
                               </td>
                               <td className="p-6">
                                  <div className="space-y-0.5">
                                      ...
                                      {client.phone && <p className="text-[10px] text-slate-400 dark:text-zinc-500">{client.phone}</p>}
                                  </div>
                               </td>
                               <td className="p-6">
                               ...
                               <td className="p-6 text-center">
                                  <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full font-black text-[10px] ${isVip ? 'bg-amber-50 text-amber-600 dark:bg-amber-500/10' : 'bg-purple-50 text-purple-600 dark:bg-purple-500/10'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                       <div className="flex items-center gap-2 mb-1">
                          ...
                          {selectedClient.totalAppointments >= vipThreshold && (
                            <div className="px-2 py-0.5 bg-amber-500 text-white text-[8px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-amber-500/20 flex items-center gap-0.5">
                       ...
                       <div className="flex flex-wrap gap-3 text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                          {selectedClient.email ? (
                            ...
                            <div className="text-[10px] italic opacity-60">Sin correo registrado</div>
                          )}
                          {selectedClient.phone ? (
                            ...
                            <div className="text-[10px] italic opacity-60">Sin teléfono registrado</div>
                          )}
                       </div>
                    </div>
                 </div>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                   <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/10">
                      <div className="flex items-center gap-2 mb-1">
                         ...
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Invertido</p>
                      </div>
                   ...
                   <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/10">
                      <div className="flex items-center gap-2 mb-1">
                         ...
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Citas</p>
                      </div>
                ...
                <div className="space-y-3">
                   <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-base font-bold text-slate-900 dark:text-white">Últimas visitas</h3>
                   </div>
                   ...
                   <div className="border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden bg-white dark:bg-zinc-800/50">
                      <table className="w-full text-left border-collapse">
                         <thead className="bg-slate-100/50 dark:bg-white/5">
                            <tr>
                               <th className="p-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Fecha</th>
                               <th className="p-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Servicio</th>
                               <th className="p-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Personal / Sucursal</th>
                               <th className="p-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Monto</th>
                            </tr>
                         </thead>
                         <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                               ...
                               <tr key={idx} className="text-xs hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                  <td className="p-3">
                                     <div className="flex flex-col">
                                        ...
                                        <span className="text-[9px] text-slate-400 font-bold">{format(h.startTime, "HH:mm")}</span>
                                     </div>
                                  </td>
                                  <td className="p-3">
                ...
                <div className="p-5 bg-gradient-to-br from-indigo-500/5 to-purple-500/5 border border-purple-500/20 rounded-[2rem]">
                   <div className="flex items-center justify-between gap-3 mb-2">
                       <div className="flex items-center gap-2 text-purple-600 dark:text-purple-400">
                          ...
                          <h4 className="text-xs font-black uppercase tracking-widest">Potencial de lealtad</h4>
                       </div>
                       <Link 
                          href={`/${locale}/admin/appearance`}
                          className="flex items-center gap-1 text-[9px] font-black text-indigo-500 hover:text-indigo-600 uppercase tracking-widest border border-indigo-200 dark:border-indigo-500/30 px-2 py-1 rounded-lg transition-all"
                       >
                          <Settings2 className="w-2.5 h-2.5" /> Ajustar umbral
                       </Link>
                   </div>
                   <p className="text-[13px] text-slate-600 dark:text-zinc-400 leading-relaxed font-medium">
                      {selectedClient.totalAppointments >= vipThreshold 
                        ? `¡Excelente! Este cliente es VIP por haber superado las ${vipThreshold} citas. Considera ofrecerle un beneficio especial.` 
                        : `Este cliente lleva ${selectedClient.totalAppointments} citas. Le faltan ${vipThreshold - selectedClient.totalAppointments} para alcanzar el estatus VIP configurado.`}
                   </p>
                </div>
              </div>
